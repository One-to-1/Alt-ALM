...
| LookupList | Fetch valid values for the field's `List-Id` at runtime (`customization/used-lists/{id}/items` — NOT the unrelated `list-items` collection), weighted-random pick per §6 | List-Ids are per-instance — never hardcode. `test-set.parent-id`/`test-set-folder.parent-id` are typed LookupList despite being tree-parent refs; still write the target folder id as the value |
